...
{
  int init_rem_len=rem_len; //printf("rem_len即remaining = %d\n",rem_len); 
  sch_subh padding; 
  padding.set_padding(); 
...
//printf("ce_only = %d\n",ce_only);  
  ...
  for (int i=0;i<nof_subheaders;i++) {//printf("testHERE2\n"); 
    header_sz += subheaders[i].get_header_size(!multibyte_padding && i==last_sdu_idx);
    if (!subheaders[i].is_sdu()) {//printf("testHERE3\n"); 
  ...
  if (rem_len + header_sz + ce_payload_sz + total_sdu_len != pdu_len) {printf("testHERE16\n"); 
    printf("\n------------------------------\n");
    for (int i=0;i<nof_subheaders;i++) {
